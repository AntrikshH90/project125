...
async function datasetInfo(id: string): Promise<{ rowsHint: number; splits: { name: string; numBytes?: number }[] }> {
  try {
    const j: any = await (await fetch(`https://datasets-server.huggingface.co/size?dataset=${id}`, { headers: authHeaders() })).json();
...
export async function streamRows(jobId: string, dataset: string, want: number, split?: string): Promise<RowsResult> {
  ...
  const splitsJ: any = await (await fetch(`https://datasets-server.huggingface.co/splits?dataset=${dataset}`, { headers: authHeaders() })).json();
...
export async function downloadParquet(jobId: string, dataset: string, want: number, split?: string): Promise<{ files: string[]; totalBytes: number; rowsAvailable: number }> {
  const j: any = await (await fetch(`https://datasets-server.huggingface.co/parquet?dataset=${dataset}`, { headers: authHeaders() })).json();
  ...
  let rowsAvailable = 0;
  try {
    const sj: any = await (await fetch(`https://datasets-server.huggingface.co/splits?dataset=${dataset}`, { headers: authHeaders() })).json();
