...
export interface LlmIntent {
  kind: "papers" | "dataset" | "github" | "images" | "web";
...
{"kind":"papers|dataset|github|images|web","query":"core topic stripped of filler words","count":<number or null>,"url":"<only if the user gave a specific URL, else null>"}

Rules:
- "papers" = research papers/publications/PDFs wanted. count = number of papers (default 20).
- "dataset" = tabular data / rows for training or analysis. count = rows wanted (default 5000).
- "github" = repositories/source code. count = repos (default 10).
...
- query: strip "get me", "give me", "I want", counts, units, "for training", etc. Keep the noun phrase.`;
  try {
    ...
    if (!["papers", "dataset", "github", "images", "web"].includes(parsed.kind)) return null;
    return parsed;
  } catch {
    return null;
  }
