...
async function llmFallback(rawInput: string): Promise<Intent | null> {
  try {
    ...
    switch (r.kind) {
      ...
      case "edu": return { kind: "edu", topic: r.query, limit: clamp(r.count || 10, 3, 60) };
      case "models": return { kind: "models", query: r.query, limit: clamp(r.count || 10, 3, 50) };
      case "generic": return { kind: "generic", query: r.query, limit: clamp(r.count || 6, 1, 20) };
      case "jobs": return { kind: "jobs", query: r.query, limit: clamp(r.count || 10, 5, 50) };
      case "data_index": return { kind: "data_index", query: r.query, limit: clamp(r.count || 15, 5, 80) };
    }
  } catch { return null; }
  return null;
}
...
const PAPER_HINTS = [
  ...
  "model", "models", "pretrained", "foundation", "transformer",
  "image generation", "image classification", "speech to text", "speech recognition",
  "audio model", "text to speech", "llm", "language model", "deep learning model",
  "neural network", "ml model", "ai model", "generative model",
];
const DATASET_HINTS = [
...
const JOB_HINTS = ["job", "jobs", "hiring", "career", "careers", "position", "opening", "hire", "recruiting", "hr", "employment", "work"];
const DATA_INDEX_HINTS = ["dataset", "datasets", "data", "sources", "data sources", "data directory", "data catalog"];
...
export function classify(rawInput: string): Intent {
  ...
  // Special check for exam patterns that contain "papers"
  ...
  const topKind = top[0][0] as "papers" | "dataset" | "github" | "images" | "web" | "edu" | "models" | "generic" | "jobs" | "data_index";
  if (top[0][1] === 0) {
  ...
  switch (topKind) {
    ...
    case "generic": {
      const query = strip(rawInput) || rawInput;
      return { kind: "generic", query, limit: clamp(count || 6, 1, 20) };
    }
    case "jobs": {
      const query = strip(rawInput) || rawInput;
...
export function describePlan(i: Intent): string {
  switch (i.kind) {
    ...
    case "edu": return `Finding exam-prep papers, syllabi, and PYQs for ${i.topic} → PDFs, markdown pages, and a clickable sources list.`;
    case "models": return `Searching GitHub for ready-built AI/ML models matching "${i.query}" + CSV, HTML comparison, and READMEs for top repos.`;
    case "generic": return `Searching the web for files about "${i.query}" (PDFs, audio, video, archives) + scraped pages.`;
    case "jobs": return `Searching job boards for "${i.query}" — postings with company, location, salary, and HR contact emails.`;
    case "data_index": return `Comprehensive dataset discovery for "${i.query}" — formats, sizes, licenses, quality ratings from HuggingFace, Kaggle, etc.`;
