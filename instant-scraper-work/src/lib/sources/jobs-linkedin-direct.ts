// sources/jobs-linkedin-direct.ts — Direct LinkedIn search scraper (no DuckDuckGo dependency).
...
const HR_EXTRACT_RE = /(?:hr[.-]?email|contact[.-]?hr|talent[.-]?acquisition|recruiting|careers|apply\?email=|@linkedin\.com|@indeed\.com|@gmail\.com|@yahoo\.|@company\.com)/i;

async function extractEmailsFromHtml(html: string): Promise<string[]> {
  const emails = html.match(EMAIL_RE) || [];
  const hrEmails = emails.filter(e => HR_EXTRACT_RE.test(e));
  return hrEmails.length ? hrEmails : emails;
}

async function scrapeLinkedInJobPage(url: string, html: string): Promise<JobRec | null> {
  ...
  return {
    ...
    salary: '',
    url,
    email: hrEmails.length ? hrEmails[0] : emails[0] || undefined,
    source: 'linkedin.com',
    posted,
  };
}
...
export async function runJobJob(jobId: string, query: string, limit: number): Promise<{ jobs: number; emails: number; sources: number }> {
  ...
  for (let qIndex = 0; qIndex < queries.length; qIndex++) {
    ...
    for (let j = 0; j < Math.min(results.length, 12); j++) {
      ...
      try {
        if ('linkedin.com/jobs/view'.includes(r.url)) {
          try {
            ...
            log(jobId, `failed to scrape LinkedIn job page: ${e.message}`);
          }
        } else if ('linkedin.com'.includes(r.url)) {
          const rec = await scrapePage(r.url, 30000);
          if (rec.textChars > 500) {
            jobs.push({
              title: rec.title,
              company: rec.title.split('-')[0].trim(),
              location: 'LinkedIn',
              salary: '',
              url: r.url,
              source: 'linkedin.com',
        ...
        log(jobId, `failed ${r.url}: ${e.message}`);
      }
  ...
  writeCsv(path.join(dir, 'jobs.csv'), ['title', 'company', 'location', 'salary', 'email', 'url', 'posted'], jobRows);
  ...
  fs.writeFileSync(path.join(dir, 'sources.html'), `<!doctype html><html><head><meta charset="utf-8"><title>Jobs — ${escapeHtml(query)}</title><style>body{font-family:system-ui,sans-serif;max-width:900px;margin:24px auto;padding:0 16px;line-height:1.5}li{margin:8px 0}</style></head><body><h1>Jobs for "${escapeHtml(query)}"</h1><p>${jobs.length} postings found${emailSet.size ? `, ${emailSet.size} HR emails extracted` : ''}.</p><h2>Jobs</h2><p><a href="jobs.csv">Download CSV</a> | <a href="hr-emails.csv">HR Emails CSV</a></p><h2>Source pages</h2><ul>${linksHtml || '<li>—</li>'}</ul></body></html>`);

  update(jobId, {
    preview: [{ jobsFound: jobs.length, emailsFound: emailSet.size, sourcePages: searchResults.length }],
