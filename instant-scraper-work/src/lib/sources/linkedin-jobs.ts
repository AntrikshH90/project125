...
// Bypasses DuckDuckGo, hits LinkedIn directly.

import fs from 'node:fs';
import path from 'node:path';
import * as cheerio from 'cheerio';
import { log, update, jobDir } from '../store';
import { scrapePage, SearchResult } from './web';
...
async function runLinkedInJobSearch(jobId: string, query: string, limit: number): Promise<{ jobs: number; emails: number; sources: number }> {
  ...
  for (let qIndex = 0; qIndex < queries.length; qIndex++) {
    ...
    for (let j = 0; j < Math.min(results.length, 12); j++) {
      ...
      try {
        // Check if it's a LinkedIn job page
        if ('linkedin.com/jobs/view'.includes(r.url)) {
          ...
          // For general LinkedIn pages, scrape content
          const rec = await scrapePage(r.url, 30000);
  ...
  const jobRows = jobs.map(j => [
    ...
    j.email || '', j.url, j.posted
  ]);
  writeCsv(path.join(dir, 'linkedin-jobs.csv'), 
    ['title', 'company', 'location', 'salary', 'email', 'url', 'posted'], jobRows);
  ...
  fs.writeFileSync(path.join(dir, 'linkedin-sources.html'), `<!doctype html>
<html><head><meta charset="utf-8"><title>LinkedIn Jobs — ${escapeHtml(query)}</title><style>
body{font-family:system-ui,sans-serif;max-width:900px;margin:24px auto;padding:0 16px;line-height:1.5}
li{margin:8px 0}
</style></head><body>
<h1>LinkedIn Jobs for "${escapeHtml(query)}"</h1>
<p>${jobs.length} postings found${emailSet.size ? `, ${emailSet.size} HR emails extracted` : ''}.</p>
<h2>Jobs</h2>
<p><a href="linkedin-jobs.csv">Download CSV</a> | <a href="linkedin-emails.csv">HR Emails CSV</a></p>
<h2>Source pages</h2>
